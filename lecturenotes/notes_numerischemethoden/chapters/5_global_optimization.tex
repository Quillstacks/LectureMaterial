% TODO: Talk more in detail about the loss landscapes in neural networks. Modern ones and easy ones. More like Shekel or more like flat planes?
...
\printchapterversion{5_global_optimization}
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            axis y line=middle,
            xlabel={$x$},
            ylabel={$f(x)$, $\hat{f}(x)$ and $\hat{f}'(x)$},
            domain=-2:12,
            samples=80,
            ymin=-9, ymax=3.5,
            xmin=-2, xmax=12,
...
\begin{itemize}
    ...
    \item How does the required effort scale with the size of the global basin when doing random restarts?
    \item How does basin hopping differ from random restarts?
