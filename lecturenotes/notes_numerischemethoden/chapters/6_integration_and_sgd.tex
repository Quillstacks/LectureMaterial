...
\begin{align*}
    h \cdot f(-0.4) &\approx \int_{-0.5}^{-0.3} f(x) \, dx \\
            ...
            &\approx 0.2 \cdot \frac{-0.47943 - 1.24658}{2} \\
            &= -0.17260
\end{align*}
\begin{align*}
    f(-0.4) &\approx \frac{-0.47943 - 1.24658}{2} \\
            &= -0.86300
\end{align*}
...
\begin{align*}
    ...
    \hat{I} &\approx h \cdot [f(-1.75) + f(-1.25)] \\
      ...
      &\approx -0.96649.
\end{align*}
The error is $|\tilde{I} - \hat{I}| \approx 0.01004$, which is smaller than the error with $n=1$ interval, illustrating how composites, or increasing the number of intervals improves the approximation.
...
\begin{align*}
    ...
    T_2 &= \frac{h}{2} [f(-2) + 2f(-1.5) + f(-1)] \\
        ...
        &\approx -0.93644.
\end{align*}
The error is $|\tilde{I} - T_2| \approx 0.02001$, roughly twice the midpoint error and on the opposite side of the true value. Both rules are second-order accurate, and for a smooth function the trapezoid error is about $-2$ times the midpoint error, so the two bracket the exact integral rather than agreeing.
...
\begin{align*}
    ...
    S_2 &= \frac{h}{3} [f(-2) + 4f(-1.5) + f(-1)] \\
        ...
        &\approx -0.95679.
\end{align*}
The error is $|\tilde{I} - S_2| \approx 0.00034$, far smaller than the trapezoid and midpoint errors. Simpson's rule integrates cubic polynomials exactly and is fourth-order accurate, so it tracks a smooth function like $\sin(x)$ over $[-2, -1]$ very closely, though not exactly.
...
\begin{align*}
    \hat{I} &= (b - a) \cdot \frac{1}{N} \sum_{i=1}^{N} f(X_i) \\
            ...
            &\approx -0.927228.
\end{align*}
The error is $|\tilde{I} - \hat{I}| \approx 0.02922$. For a stochastic estimator the meaningful yardstick is not another quadrature rule but its own standard error, the $1/\sqrt{N}$ spread of the estimate, which we work out next. The strength of Monte Carlo is not accuracy at small $N$ but that this error scales independently of the dimension.
...
\begin{align*}
    \sigma_f^2 &= Var(f(\mathbf{X}))  \\ 
               &\approx 0.003018, \\
    \sigma_{f} &\approx 0.05493, \\
    \mathrm{SE}(\hat I_N) &\approx \frac{|\Omega|\, \sigma_f}{\sqrt{N}} \\
    &= \frac{1\cdot 0.05493}{2} \\
    &\approx 0.02747.
\end{align*}

The observed error of $0.02922$ is about one standard error, just as the $1/\sqrt{N}$ scaling predicts.
...
Briefly elaborate on how the error of the Monte Carlo estimator is dimension independent.
