...
\printchapterversion{3_erroranalysis}
...
use a simplified Manhattan or Euclidean model that does not account for terrain, mode of travel, or obstacles. So keep in mind that our $f(\cdot)$ here, is another $\hat{f}(\cdot)$.}
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Continuous curve $\hat{f}(x) = \sin(x)$ on $[-3,1]$ with two highlighted transparent vertical bands at $x=-1.5$ and $x=0$ (width $0.5$).}
...
in the computed values of $\hat{f}(\tilde{x})$.
...
A numerical method is convergent if, as we refine our approximation $\hat{f}(x)$ (for example, by decreasing the step size $h$), 
...
and let $\hat{f}(\cdot)$ represent the numerical method (algorithm) that provides an approximation.
...
\marginnote{\newthought{Hat versus tilde.} The hat $\hat{\cdot}$ marks the numerical method (algorithm), while the tilde $\tilde{\cdot}$ marks a perturbed input. So $\hat{f}(\tilde{x})$ reads as: numerical method evaluated on perturbed input data.}
...
Mathematically, a stable method ensures that the error in the computed solution $\hat{f}(\tilde{x})$ remains bounded by a constant multiple of the error in the input data:

\begin{equation}
    \left| \hat{f}(\tilde{x}) - \hat{f}(x) \right| \leq s \cdot \left| \tilde{x} - x \right|,
\end{equation}
...
\begin{equation}
   \left| \hat{f}(\tilde{x}) - f(x) \right| \to \lim
\end{equation}
...
\begin{marginfigure}
    ...
    \begin{tikzpicture}
        ...
        \draw[gray, densely dashed] (axis cs:-1.5708,-1.25) -- (axis cs:-1.5708,0);
        \draw[gray, densely dashed] (axis cs:-3.3,-1) -- (axis cs:-1.5708,-1);
        \addplot[black, thick, smooth] {sin(deg(x))};
        \addplot[gray, only marks, mark=*, mark size=0.6pt] coordinates {
            (-3,-0.14112) (-2.8,-0.33499) (-2.6,-0.5155) (-2.4,-0.67546)
            (-2.2,-0.8085) (-2,-0.9093) (-1.8,-0.97385) (-1.6,-0.99957)
            (-1.4,-0.98545) (-1.2,-0.93204) (-1,-0.84147) (-0.8,-0.71736)
            (-0.6,-0.56464) (-0.4,-0.38942) (-0.2,-0.19867) (0,0)
            (0.2,0.19867) (0.4,0.38942) (0.6,0.56464) (0.8,0.71736) (1,0.84147)
        };
        \addplot[black, only marks, mark=o, mark size=1.6pt] coordinates {
            (-3,-0.14112) (-2,-0.9093) (-1,-0.84147) (0,0) (1,0.84147)
        };
        \addplot[black, only marks, mark=*, mark size=1.4pt] coordinates {(-2,-0.9093) (-1.6,-0.99957)};
        \node[gray, font=\tiny, anchor=south west] at (axis cs:-1.5708,0) {$x^\ast$};
        \end{axis}
    \end{tikzpicture}
    \caption{Reference grid for the exercise: $\sin(x)$ on $[-3,1]$ with the $h=1$ grid (open circles)
    and the $h=0.2$ grid (dots). The true minimum sits at $x^\ast = -\frac{\pi}{2}$ with $f(x^\ast) = -1$ (dashed).
    The nearest grid point, which is also the grid minimum, is $x=-2$ for $h=1$ and $x=-1.6$ for $h=0.2$ (filled).}
...
We can see that around $x=-1$, near the minimum at $x^\ast = -\frac{\pi}{2}$, the sine function is relatively flat, indicating good conditioning. For sake of comparison we also consider
$x=0$, where the sine function changes rapidly, indicating poor conditioning. 
...
\begin{align*}
    ...
    &= 0.2474 \\
\end{align*}
Which shows ill-conditioning with $\kappa = 0.2474$.




\newthought{Stability} depends on the numerical method used to approximate the sine function and find its minimum.
We probe stability at the working point $x=-1$, the same point used for conditioning, near the minimum at $x^\ast = -\frac{\pi}{2}$. For the discretization with step size $h=1$, the method is stable as small changes in $\tilde{x}$ lead to small changes in the computed values of $\hat{f}(\tilde{x})$.
For the discretization with step size $h=0.2$, the method is less stable, as small changes in $\tilde{x}$ can lead to larger fluctuations in the computed values of $\hat{f}(\tilde{x})$.
...
At $x=-1$ the sine curve is not symmetric, so the two perturbation directions give different errors and we consider both $\tilde{x} = x \pm 0.25$.
\marginnote{\newthought{a)} See how, for $h=1$, both perturbed inputs round back to $-1$, leaving the value unchanged. For reference, Fig.~\ref{fig:grid_reference}}

\begin{align*}
    s^{+}_{h=1} \cdot |\tilde{x} - x| &= |\hat{f}_1(-0.75) - \hat{f}_1(-1)| \\
    &= |\hat{f}_1(-1) - \hat{f}_1(-1)| \text{ see A)} \\
    &= 0 \\
    s^{+}_{h=1} &= 0 \div 0.25 \\
    &\approx 0 \\[1em]
    s^{-}_{h=1} \cdot |\tilde{x} - x| &= 0 \\
    s^{-}_{h=1} &\approx 0
\end{align*}
\marginnote{\newthought{One side suffices.} When the perturbation $0.25$ stays below half a grid step, $\tilde{x} = x \pm 0.25$ rounds back to the same grid point on both sides, so the two directions coincide. For $h=1$ the half-step is $0.5 > 0.25$; for $h=0.2$ it is only $0.1 < 0.25$, which is why $h=0.2$ needs both sides.}

\vspace{1em}
For $h=0.2$ the two directions differ, so we evaluate both:
\begin{align*}
    s^{+}_{h=0.2} \cdot |\tilde{x} - x| &= |\hat{f}_{0.2}(-0.75) - \hat{f}_{0.2}(-1)| \\
    &= |\hat{f}_{0.2}(-0.8) - \hat{f}_{0.2}(-1)| \\
    &= |-0.71736 - (-0.84147)| \\
    &= 0.12411 \\
    s^{+}_{h=0.2} &= 0.12411 \div 0.25 \\
    &\approx 0.4964 \\[1em]
    s^{-}_{h=0.2} \cdot |\tilde{x} - x| &= |\hat{f}_{0.2}(-1.25) - \hat{f}_{0.2}(-1)| \\
    &= |\hat{f}_{0.2}(-1.2) - \hat{f}_{0.2}(-1)| \\
    &= |-0.93204 - (-0.84147)| \\
    &= 0.09057 \\
    s^{-}_{h=0.2} &= 0.09057 \div 0.25 \\
    &\approx 0.3623
\end{align*}
\marginnote{\newthought{Border regions.} The two directions bracket the stability constant, so $0.3623 \leq s_{h=0.2} \leq 0.4964$.
 Stability has anomalies in the border regions of the discretization. This is especially a problem for piece-wise constant approximations.}




 
\newthought{Consistency} is determined by comparing the numerical solution $\hat{f}(x)$ with the exact solution $f(x)$ for the minimum of the sine function in $[-3,1]$.
...
For $h=1$:
\begin{align*}
    c_1 &\geq \left| \hat{f}_1(-\frac{\pi}{2}) - f(-\frac{\pi}{2}) \right| \\
    &\geq \left| \hat{f}_{1}(-2) - f(-\frac{\pi}{2}) \right| \\
    &\geq \left| -0.90930 - (-1) \right| \\
    &\geq 0.0907
\end{align*}
...
For $h=0.2$:
\begin{align*}
    c_{0.2} &\geq \left| \hat{f}_{0.2}(-\frac{\pi}{2}) - f(-\frac{\pi}{2}) \right| \\
    &\geq \left| \hat{f}_{0.2}(-1.6) - f(-\frac{\pi}{2}) \right| \\
    &\geq \left| -0.99957 - (-1) \right| \\
    &\geq 0.00043
\end{align*}
...
To quantify convergence we calculate the total error between the numerical solution $\hat{f}(\tilde{x})$ and the exact solution $f(x)$:
...
\vspace{1em}
We perturb the minimizer, $\tilde{x} = -\frac{\pi}{2} \pm 0.25$, and discretize. Because the discretization rounds $\tilde{x}$ 
to a different grid point in each direction, the two sides differ and we evaluate both.

For $h=1$:
\begin{align*}
    \left| \hat{f}_1(\tilde{x}) - f(x) \right| &= \left| \hat{f}_1(-1.32) - f(-\frac{\pi}{2}) \right| \\
    &= \left| \hat{f}_1(-1) - f(-\frac{\pi}{2}) \right| \\
    &= \left| -0.84147 - (-1) \right| \\
    &= 0.15853 \\[1em]
    \left| \hat{f}_1(\tilde{x}) - f(x) \right| &= \left| \hat{f}_1(-1.82) - f(-\frac{\pi}{2}) \right| \\
    &= \left| \hat{f}_1(-2) - f(-\frac{\pi}{2}) \right| \\
    &= \left| -0.90930 - (-1) \right| \\
    &= 0.0907
\end{align*}
so $0.0907 \leq \left| \hat{f}_1(\tilde{x}) - f(x) \right| \leq 0.15853$.

\vspace{1em}
For $h=0.2$:
\begin{align*}
    \left| \hat{f}_{0.2}(\tilde{x}) - f(x) \right| &= \left| \hat{f}_{0.2}(-1.32) - f(-\frac{\pi}{2}) \right| \\
    &= \left| \hat{f}_{0.2}(-1.4) - f(-\frac{\pi}{2}) \right| \\
    &= \left| -0.98545 - (-1) \right| \\
    &= 0.01455 \\[1em]
    \left| \hat{f}_{0.2}(\tilde{x}) - f(x) \right| &= \left| \hat{f}_{0.2}(-1.82) - f(-\frac{\pi}{2}) \right| \\
    &= \left| \hat{f}_{0.2}(-1.8) - f(-\frac{\pi}{2}) \right| \\
    &= \left| -0.97385 - (-1) \right| \\
    &= 0.02615
\end{align*}
so $0.01455 \leq \left| \hat{f}_{0.2}(\tilde{x}) - f(x) \right| \leq 0.02615$.

\newthought{Convergence improves} with smaller step sizes: the $h=0.2$ interval $[0.01455, 0.02615]$ lies well below the $h=1$ interval $[0.0907, 0.15853]$.
Each total error is at least the consistency error and grows by the stability contribution once the perturbation pushes the discretized estimate into a neighbouring cell.
Convergence thus combines consistency and stability, exactly as its definition promises.
...
\begin{itemize}
    \item Compared to the problem of numerically finding the minimum of the sine function, how well conditioned is the two-equation system?
