...
% background live in marginnotes. Every part cites its datasheet. No prices anywhere,
% neither in the part sections nor in the chapter opening, so the chapter cannot go
% stale line by line.
%
...
%
% Scope stops at the wired machine. BIOS setup is the first thing software does
% to the rig, so it lives in the Stack chapter,
...
\begin{itemize}[leftmargin=*, topsep=0.25em, itemsep=0.1em, parsep=0em]
    \item A6000, 48~GB: Twice the VRAM at ten times the price.
...
Skip to \hyperref[ch:stack]{the next chapter} for the BIOS setup and the first
boot of the rig.
...
parallelism\index{tensor parallelism} moves activations every token and starts to crawl. So the idea is to have the same model loaded
    on each card, and let the scheduler round robin\index{round robin} between them for throughput.
...
container runtime, same Ollama binary on the same port, same clients pointed at it.
...
your bench. What it still lacks is a first power on and a handful of BIOS values
that make a card addressable, and then an operating system, a driver, and weights
loaded into memory together with some orchestration. Let's get on it.
