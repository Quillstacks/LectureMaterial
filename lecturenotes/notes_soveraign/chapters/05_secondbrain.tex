...
your vault. Point your notes at Ollama and it answers from your own record
...
private path from the vault to the rig a matter of pointing a plugin at an
address.}

\newthought{The tool is Obsidian} with two community plugins\index{Obsidian!plugins}: one to chat with the
vault, one to find related notes by meaning. Both point at the Chapter~2 Ollama over
...
\begin{itemize}
    \item pulled an embedding model and exposed it through Ollama;
...
\newthought{Where the work runs.}\index{tailnet} Obsidian runs on your laptop, Ollama on the
rig. The plugins reach it at its address on your tailnet, so indexing and
...
\newthought{Pull the embedding model}\index{qwen3-embedding} on the rig, alongside the chat model:

\begin{verbatim}
$ ollama pull qwen3-embedding:0.6b   # made for embeddings
...
on the rig so Ollama accepts requests from the Obsidian app, the one piece of
...
\newthought{Ollama now answers from your own record}, not only from its
