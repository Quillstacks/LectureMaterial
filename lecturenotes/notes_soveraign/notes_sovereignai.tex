...
%
% Series spine (chapters 03-06 are currently commented out below):
%   01 The Rig               hardware, design for four, one 3090, wired and unpowered
%   02 The Stack             first boot, Ubuntu Server, driver, Docker, Ollama on the API
%   03 Everywhere            Tailscale mesh, tailnet identity as the credential
%   04 Screens and Voices    Open WebUI, Cloudflare door, Home Assistant voice
%   05 Second Brain          Obsidian on the local Ollama
...
\title[A Hitchhiker's Guide to Sovereign AI at Home]{Why do I not own my neural network?\\
{\fontsize{16}{20}\selectfont A Hitchhiker's Guide to Sovereign AI at Home}}
...
% A \cite reprints the whole bib entry in the margin, so natbib's year-letter
% disambiguation, 2026a and 2026b, adds nothing here. The .bbl wraps exactly
% those letters in \natexlab. natbib resets \natexlab to identity from its own
% \AtBeginDocument hook, registered at class load, so this later hook runs
% after it and gobbles the letters: The years print plain.
\AtBeginDocument{\def\natexlab#1{}}
...
\makeintroduction{%
  ...
  PCIe~3.0 lane can keep up. If you are like me and you train your own AI models, do 
  fine tuning\index{fine-tuning} and image or video
  generation\index{image generation} this setup is not for (you) that. Those things move tensors across 
  the bus for the whole run. 
  When facing the latter use-case you should target a server platform auch as an AMD EPYC\index{AMD EPYC} board, dedicated cards a full x16 link each.}
...
% \input{chapters/01_hardware.tex}

% CHAPTER 2: the stack, first boot, OS, driver, Docker, Ollama on the API
\input{chapters/02_stack.tex}

% CHAPTER 3: everywhere, Tailscale mesh, tailnet identity as the credential
%\input{chapters/03_everywhere.tex}

% CHAPTER 4: screens and voices, Open WebUI, Cloudflare door, Home Assistant voice
%\input{chapters/04_screensvoices.tex}

% CHAPTER 5: second brain, Obsidian on the local Ollama
%\input{chapters/05_secondbrain.tex}

% CHAPTER 6: next steps, multimodal, OCR, retrieval, images, and beyond
%\input{chapters/06_nextsteps.tex}
