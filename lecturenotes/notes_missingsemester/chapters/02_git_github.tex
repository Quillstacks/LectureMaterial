...
If you see a file called \texttt{id\_ed25519}, that path is already in use. Either reuse it
for GitHub as well, or generate a new key under a distinct filename so both keep working.
Otherwise, generate the default key pair on the \texttt{dev} VM:
\marginnote{\newthought{Multiple SSH keys?}\index{SSH!config}
If you keep one key for production servers and another for GitHub, tell SSH which to use
for each host with an \texttt{\textasciitilde/.ssh/config} entry:
\\[0.5em]
\texttt{Host github.com}\\
\texttt{\quad HostName github.com}\\
\texttt{\quad User git}\\
\texttt{\quad IdentityFile \textasciitilde/.ssh/id\_ed25519\_github}\\
\texttt{\quad IdentitiesOnly yes}
\\[0.5em]
Then \texttt{chmod 600 \textasciitilde/.ssh/config}. Without
\texttt{IdentitiesOnly}, SSH may offer the wrong key first and GitHub will reject the
connection after a handful of auth failures.}
\begin{verbatim}
ssh-keygen -t ed25519 -C "your@email.com"
\end{verbatim}

The tool prompts for a file location. If \texttt{\textasciitilde/.ssh/id\_ed25519} does not yet
exist, press Enter to accept the default; if it does, type a distinct path such as
\texttt{\textasciitilde/.ssh/id\_ed25519\_github} so the existing key is preserved.
Depending on your setup, you might need to use an absolute path like \texttt{/home/<user>/.ssh/id\_ed25519\_github} .
It then prompts for a passphrase, optional but recommended, it encrypts the private key on
disk so a stolen key file is not immediately usable.
...
\texttt{id\_ed25519} is the private key: never share it, never commit it, never copy it
to another machine without good reason.
\texttt{id\_ed25519.pub} is the public key, safe to share, this is the file you paste into GitHub.
The \texttt{-t ed25519} flag picks a modern signature algorithm; the \texttt{-C} comment
labels the key so you can recognise it later when you have several.}
...
Save with \texttt{Ctrl+O} and exit with \texttt{Ctrl+X}, then complete the merge:
