...
This block builds on the state at the end of Block~4: A working classifier
in \texttt{app/classifier.py}, a smoke test in \texttt{predict.py}, the
model artefact in \texttt{models/}, and \texttt{MODEL\_PATH} pinned in
\texttt{.env}.
If you joined late, missed Block~4, or your repository has drifted,
restore from the snapshot and reset to \texttt{v0.3} rather than redoing
...
On both VMs, restore the \texttt{B4-complete} snapshot from Block~4.
The snapshot carries everything that is not in Git, the \texttt{.venv/},
the model artefact \texttt{digit\_classifier\_v1.pkl} in \texttt{models/},
the system packages from \texttt{setup-server.sh}, and any real
...
This rewrites your working tree to match the end of Block~4 exactly,
including \texttt{app/classifier.py}, \texttt{predict.py}, and
\texttt{.env.example}.
There is no work to lose if you have not committed beyond \texttt{v0.3};
...
If your own fork carries the \texttt{v0.3} tag, swap \texttt{origin} to
...
\newthought{Without a snapshot,} replay Block~4 before continuing.
The file state alone is not enough when the next exercise expects an
active virtual environment, the model artefact downloaded into
\texttt{models/}, and \texttt{MODEL\_PATH} set in \texttt{.env}.
You are now at the state where Block~4 ended, with both machines
aligned, ready to start Block~5.
...
\marginnote{\newthought{Exercises} are where the theory above turns into
keystrokes.
Each theory section is followed by its exercise, so you can read a concept
and practise it immediately.
At the end of the session, take a snapshot named \texttt{B5-complete}, your
safety net for the next block.}
...
\newthought{Turn the contract above into code.}
