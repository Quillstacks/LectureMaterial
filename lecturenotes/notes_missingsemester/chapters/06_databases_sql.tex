...
This block builds on the state at the end of Block~5: A FastAPI service
exposing \texttt{POST /classify} and \texttt{GET /health}, with API-key
authentication and basic rate limiting, running under a \texttt{systemd}
unit on \texttt{prod}, with request and response contracts defined as
Pydantic models.
If you joined late, missed Block~5, or your repository has drifted,
restore from the snapshot and reset to \texttt{v0.4} rather than redoing
...
On both VMs, restore the \texttt{B5-complete} snapshot from Block~5.
The snapshot carries everything that is not in Git, the \texttt{.venv/},
the model artefact in \texttt{models/}, the system packages from
\texttt{setup-server.sh}, the \texttt{pixelwise.service} systemd unit
installed on \texttt{prod}, and any real \texttt{.env} values you wrote.
...
This rewrites your working tree to match the end of Block~5 exactly,
including \texttt{app/main.py} and the updated \texttt{requirements.txt}.
There is no work to lose if you have not committed beyond \texttt{v0.4};
...
If your own fork carries the \texttt{v0.4} tag, swap \texttt{origin} to
...
\newthought{Verify the service.}
On \texttt{prod}, confirm the FastAPI service is reachable before moving
on:
...
A \texttt{200 OK} response confirms the systemd unit is running against
the rewound code.
You are now at the state where Block~5 ended, with both machines
aligned, ready to start Block~6.

\newthought{Without a snapshot,} replay Block~5 before continuing.
The file state alone is not enough when the next exercise expects an
installed systemd unit and a running API service.
...
\marginnote{\newthought{Exercises} sit next to the theory they exercise.
Each pair takes you from concept to running command on the same topic, and
the snapshot from Block~1 is your safety net if a step goes wrong.}
...
With the migration cycle from the theory above in mind, configure the tool
to use the same engine as the application:
