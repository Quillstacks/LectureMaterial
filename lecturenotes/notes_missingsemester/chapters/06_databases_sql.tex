...
the model to the database. \texttt{create\_engine} reads the
\texttt{DATABASE\_URL} from the environment and opens a pool of connections;
...
in \texttt{.env} for access restrictions and security.
...
\newthought{Write \texttt{app/models.py}} on \texttt{dev} with the
schema for one prediction, the class label, the confidence, and the
model version:
...
\newthought{Document \texttt{DATABASE\_URL}} in
\texttt{\textasciitilde{}/pixelwise/.env.example} on \texttt{dev}, and
add the same line to your local \texttt{.env} on both VMs so the engine
can find the database. The URL interpolates \texttt{DB\_PASSWORD} from
the same file, so the secret lives in one place:
...
\texttt{Prediction} is the row class you defined in \texttt{app/models.py};
\texttt{SessionLocal} is the session factory built from the engine. Add
the line at the top of \texttt{main.py} next to the existing imports.}
