...
\texttt{pytest tests/} from \texttt{/opt/pixelwise} and refuses to
...
The same \texttt{pytest tests/} invocation that you just did locally and we will gate deployment
...
\texttt{pytest tests/} should print two passes.
...
\begin{itemize}
    ...
    \item The classroom pipeline is three primitives: a systemd timer as the trigger, \texttt{pytest tests/} as the gate, and one bash script as the deploy step.
