...
Add the following at the bottom of the file, save with \texttt{Ctrl+O},
and exit with \texttt{Ctrl+X}:
...
cp .env.example .env       # if you have not yet
bash setup-server.sh
ls models/
\end{verbatim}
...
\marginnote{\newthought{Why \texttt{parser="liac-arff"}}.\index{scikit-learn}\index{pandas}
Since scikit-learn 1.2, \texttt{fetch\_openml} defaults to \texttt{parser="auto"}, which prefers a pandas-based parser and raises if pandas is not installed. Naming the built-in liac-arff parser keeps the dependency footprint at what \texttt{requirements.txt} pins.}
...
restart, the same pattern as the database URL in Block~6 and the API
key from Block~5.
...
Paste the implementation below into the empty file, save with
\texttt{Ctrl+O}, exit with \texttt{Ctrl+X}. It includes the class
...
call \texttt{classify\_batch}, no HTTP endpoint, no \texttt{systemd}
unit, no entry point at all. Pulling the new code there now would just
add an unused \texttt{app/classifier.py} to the production checkout.
Block~5 introduces the FastAPI layer and the \texttt{systemd} service
that make the model reachable; deployment to \texttt{prod} happens then,
