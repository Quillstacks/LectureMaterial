...
\begin{figure}
    ...
    \node[box] (bin)   at (2.2, 0)  {binarize\\\texttt{>128}};
    \node[box] (flat)  at (4.4, 0)  {flatten\\\texttt{(1,784)}};
    \node[box] (model) at (6.6, 0)  {model\\\texttt{.pkl}};
    \node[box] (pred)  at (8.8, 0)  {predict\\\texttt{\_proba}};
    \node[box] (out)   at (11.0, 0) {result\\\texttt{dict}};
    \draw[arr] (img.east)   -- (bin.west);
    \draw[arr] (bin.east)   -- (flat.west);
    \draw[arr] (flat.east)  -- (model.west);
    \draw[arr] (model.east) -- (pred.west);
    \draw[arr] (pred.east)  -- (out.west);
    \end{tikzpicture}
    \caption{\rule{0pt}{1.4cm}\\
    The inference pipeline for one image: a raw \texttt{(28,28)} grayscale array is binarised at threshold 128, flattened into a \texttt{(1,784)} row vector, fed to the loaded \texttt{.pkl} model, and turned into a result dictionary by \texttt{predict\_proba}.}
...
This block builds on the state at the end of Block~3: A provisioned VM
with the virtual environment, pinned dependencies, the \texttt{app/}
scaffold, and \texttt{.env.example} in place.
If you joined late, missed Block~3, or your repository has drifted,
restore from the snapshot and reset to \texttt{v0.2} rather than redoing
...
On both VMs, restore the \texttt{B3-complete} snapshot from Block~3.
The snapshot carries everything that is not in Git, the \texttt{.venv/},
the system packages installed by \texttt{setup-server.sh}, and any real
...
including \texttt{setup-server.sh}, \texttt{requirements.txt}, and
\texttt{.env.example}.
There is no work to lose if you have not committed beyond \texttt{v0.2};
...
If your own fork carries the \texttt{v0.2} tag, swap \texttt{origin} to
...
\newthought{Without a snapshot,} replay Block~3 before continuing.
The file state alone is not enough when the next exercise expects an
active virtual environment and the system Python packages from
\texttt{setup-server.sh}.
You are now at the state where Block~3 ended, with both machines
aligned, ready to start Block~4.
...
The course hosts \texttt{digit\_classifier\_v1.pkl} as a tagged release in
...
Then download the tagged artefact into \texttt{models/} using the GitHub
CLI; the same call works locally and inside CI:
...
\texttt{git status},} your \texttt{.gitignore} from Block~3 needs an update.
...
\begin{marginfigure}
    \raggedleft
    \includegraphics[width=0.55\marginparwidth]{./figures/mnist_eight.png}
...
The Pipeline object exposes its named steps as a dictionary, and the
classifier exposes the classes it knows:
...
\marginnote{\newthought{Reading is the exercise.}\index{model card}
Authoring a card for a model that already ships with one is busywork.
Authoring a card for a model you publish yourself is load-bearing
documentation, and that is where the writing exercise lives, in the
optional v2 exercise at the end of this block.}
...
Block~5 swaps stdin for HTTP and stdout for JSON, and moves the service
to \texttt{prod}; the inference logic itself is unchanged.}

\newthought{Run the smoke test on dev} and commit:
...
a dev-side smoke test, and an updated configuration contract pinned to the
model release.
...
Block~5 takes this dev-verified module and exposes it as an HTTP service
on \texttt{prod}.
...
On the PixelWise side, bump \texttt{MODEL\_REPO=<you>/pixelwise-model}
and \texttt{MODEL\_VERSION=v2.0} in \texttt{.env} and re-run
\texttt{gh release download}.
...
cannot reach it; \texttt{prod} has not yet seen this artefact.
...
managed \texttt{systemd} service on \texttt{prod} so the model becomes
