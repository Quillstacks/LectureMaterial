...
\marginnote{\newthought{Optional: pin pandas.}\index{pandas}\index{requirements.txt}
\texttt{fetch\_openml} pulls in \texttt{pandas} under the hood, and we
have leaned on it implicitly more than once already. If you want it as
an explicit, pinned dependency rather than a transitive one, install
and refreeze:
\\[0.5em]
\texttt{pip install pandas}\\
\texttt{pip freeze > requirements.txt}\\
\texttt{git add requirements.txt}\\
\texttt{git commit -m "Pin pandas"}\\
}
