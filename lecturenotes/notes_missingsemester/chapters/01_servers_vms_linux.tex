...
% 1. Add a figure of the two-machine architecture (dev and server on a host-only network).
% 2. Record a short walkthrough of the SSH handshake and link it in a margin note.
...
\newthought{It's like Matrix, but for software.}
...
\marginnote{\newthought{Oracle VirtualBox Manager} at \url{https://www.oracle.com/virtualization/virtualbox/}
is the hypervisor we install on the host. Virt-manager and VMWare serve the same role with different trade-offs,
and we could run this course on any of them.}
...
\newthought{Install the Oracle VirtualBox Manager} for your host OS with default settings, and download two Ubuntu LTS ISOs: the Desktop edition for \texttt{dev} and the Server edition for \texttt{prod}. The asymmetry is deliberate, \texttt{dev} carries a graphical environment so you can run an editor and a browser locally, while \texttt{prod} stays headless because a production server has no business running a desktop.
You can also decide to run dev on your machine directly, but this is not further supported in the excercises.

\marginnote{\newthought{Where to get them.} VirtualBox: \url{https://www.oracle.com/virtualization/virtualbox/}. Ubuntu Desktop LTS ISO: \url{https://ubuntu.com/download/desktop}. Ubuntu Server LTS ISO: \url{https://ubuntu.com/download/server}. Pick the LTS release in both cases, not the interim one, so support extends across the semester.}
...
\marginnote{\newthought{Host-only network.} In \texttt{File}, \texttt{Host Network Manager} (or \texttt{File}, \texttt{Tools}, \texttt{Network Manager} on version~7+), create a host-only network if none exists, typically $192.168.56.1/24$. \textbf{Disable DHCP} for this network so you can assign static IPs manually.}
...
Decode the first column of \texttt{ls -la} against the margin note on mode strings from Block~2. Name aloud what each of the ten characters means; if you can do that without looking, permissions have stopped being mysterious.
...
\marginnote{\newthought{Teaser.} Every command we ran today was reproducible, but none of it was reproduced.
How do we capture the steps so that any fresh VM can be brought to the same state without human memory?}
\newthought{So far, we owned the box.} The next step is to own the history of every change we make to it.
In the next chapter we put code and configuration under version control with Git and GitHub,
turn today's manual setup into a script, and make what we just did something a teammate, or a fresh VM, can replay from scratch.
