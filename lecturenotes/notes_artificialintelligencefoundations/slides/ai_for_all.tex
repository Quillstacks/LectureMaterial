...
%%
%% ONE PAGE, ONE FRAME, ONE QUOTE. A comic page becomes a frame. The frame
%% title is that page's own heading, the body is ONE passage lifted verbatim
%% from the page, and the right third is the page's artwork. Nothing is
%% paraphrased: the essay argues in its own voice or not at all. The one
%% exception is page 48 (ART), which carries two frames because the acronym
%% and the demand it makes are two separate claims.
%%
...
%% in teaching material as long as the credit stays on the slide, which is
%% what the \slidesources marker on every frame is for.
...
\begin{frame}{How to read this deck}
  ...
  \begin{sidebody}
    ...
    One comic page, one slide, \gterm{one quoted passage}. Nothing on the
    following slides is my wording. The frame title is the page's own
    heading, the passage is lifted verbatim, and the page number under it
    tells you where to read the rest.
    \vskip1.0em
...
\input{part2_basics.tex}
\input{part3_examples.tex}
\input{part4_chances.tex}
\input{part5_risks.tex}
\input{part6_outlook.tex}
\input{part7_utopias.tex}
