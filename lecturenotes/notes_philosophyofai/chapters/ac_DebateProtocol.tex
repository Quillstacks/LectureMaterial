...
\printchapterversion{ac_DebateProtocol}
...
Each team has \textbf{3 to 4 members}. Two chairs are placed facing each other at
...
semi-arc \textit{directly behind} their speaker, facing the opposing pair. The
...
\noindent A speaker may tag themselves out at any point by saying \textit{``I
...
\textit{No handshake with the opponent;} this is a team substitution, not a
challenge. A voluntary pass does \textit{not} count against the rotation
...
\begin{enumerate}
    \item A simple majority of the team members \textit{not in the chair} raise
...
not used to change the chair; they are used to regulate the \textit{quality}
...
\begin{enumerate}
    \item The speaker raises an open hand (or a white card) and says:
          \textit{``I concede: [state the opponent's point clearly and
          precisely].''} Vague concessions are not valid.
    \item The speaker must then pivot: \textit{``\ldots\ my position survives
    ...
    \item The conceding speaker is \textbf{protected from a red flag for
          ...
          safe to demonstrate.
    \item The opponent \textit{may not repeat or pile on the conceded point}
