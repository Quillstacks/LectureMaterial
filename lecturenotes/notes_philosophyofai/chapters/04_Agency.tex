...
\marginnote{%
    ...
    {\color{black}\qrcode[height=1.2cm]{https://en.wikisource.org/wiki/R._U._R._(Rossum_Universal_Robots)}}
    \end{flushright}%
    \vspace{1em}
    \begin{flushright}
    ...
    J.W. von Goethe (1798)\\
    Full text via Wikisource:\\[4pt]
    {\color{black}\qrcode[height=1.2cm]{https://de.wikisource.org/wiki/Der_Zauberlehrling_1798}}
    \end{flushright}%
}

\marginnote{\newthought{Case.}
    \begin{enumerate}
        \item 6 May 2010: automated algorithms trigger the Flash Crash; Dow drops
              1{,}000 points in minutes; \$1~trillion in market value erased.
        \item Market recovers within the hour. No human decided to sell at scale;
              no human decided to stop.
        \item 2015: one trader in London charged with market manipulation, arrested
              at home in his pyjamas.
    \end{enumerate}
}
...
\marginnote{\newthought{Corrigibility.} A system's willingness to be corrected or shut down. Fully corrigible:
does whatever instructed. Fully autonomous: acts on its own values. Deployed AI sits somewhere
between, and that position is a design choice.}
\marginnote{\newthought{Asimov's Three Laws.\cite{asimov1950robot}} (1)~A robot may not injure a human being, or through inaction allow a human being to come to harm. (2)~A robot must obey orders given by humans, except where this conflicts with Law~1. (3)~A robot must protect its own existence, except where this conflicts with Laws~1 or~2. Asimov spent his career writing stories about how these laws fail: conflicts, misinterpretation, and edge cases that no ranked rule set can anticipate.}
...
\marginnote{\newthought{Democratic Legitimacy.} Consequential decisions must be traceable to a mandate from
those affected. Delegating distributive policy to an algorithm without a clear mandate severs
this chain entirely.}
\medskip
...
correctly; the failure is upstream goal specification and oversight
architecture; per-decision human authorisation is not the answer,
better design is.
\vspace{1.5em}
...
\marginnote{\newthought{Goal Misspecification.} When the objective a system optimises diverges from the
actual human intent. R.U.R.'s robots optimise for collective survival; the Zauberlehrling's
brooms optimise for water-fetching. Neither intends harm.}
\medskip
...
\marginnote{\newthought{Teaser.} The four themes of this course are not independent. An agent that looks
human, acts without oversight, in physical space, with no clear accountability chain, concentrates
all four challenges at once. What governance architecture would you build to contain it?}
