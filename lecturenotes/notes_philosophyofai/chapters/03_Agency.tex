...
\newthought{R.U.R.\ coined the word} \textit{robot}\index{Robot} and traces the full
...
\newthought{Agent.}\index{Agent} From Latin \textit{agere}, to act: one who acts.
...
observation in this way was introduced for language models as ReAct\index{ReAct}
...
\begin{tabular}{p{3.8cm} p{6.5cm}}
    \textbf{Required reading} &
        \textit{R.U.R.\ (Rossum's Universal Robots)}\index{R.U.R.}\index{Capek, Karel@\v{C}apek, Karel}
...
\begin{enumerate}
    ...
    \item \textbf{Corrigibility}\index{Corrigibility} refers to a system's willingness to
...
\newthought{Human in the loop.}\index{Human in the loop} System proposes; a human
authorises each consequential action before it executes. Control per
decision, throughput bounded by human attention.

\newthought{Human on the loop.}\index{Human on the loop} System acts autonomously; a
...
of the loop}\index{Human out of the loop}: No human is positioned to intervene at all.
...
\marginnote{\newthought{Human and Automation bias.}\index{Automation bias} Deferring to automated output
and ceasing to scrutinise it, strongest when the system is usually right. Or the problem is complex.}


\marginnote{\newthought{Moral crumple zone.}\index{Moral crumple zone}\cite{elish2019crumple} A
human held responsible at the controls of a system they cannot fully
govern, positioned to absorb the blame when it fails.}

\newthought{Meaningful human control}\index{Meaningful human control}, asks not that a human be present but that a human can still decide, which
...
\marginnote{\newthought{Controllability.}\index{Controllability} How far a human can direct and
...
\marginnote{\newthought{Three Laws of Robotics.}\index{Three Laws of Robotics}\index{Asimov, Isaac} Isaac Asimov,
...
\marginnote{\newthought{Alignment.}\index{Alignment} The current research programme's name
...
machine.\index{Russell, Stuart} The gap between the specification and the intent is where the alignment gap is.
