...
\newthought{Inside the Feature Space}
...
In supervised learning\cite{DeepLearningBook}, the learning process relies on data samples $x$ paired with their ground truth labels $\tilde{y}$, which allows a 
...
without relying on external supervision, thus mostly referred to as unsupervised learning.
...
\newthought{A distance function} $d: X \times X \to \mathbb{R}_{\geq 0}$, which means taking two samples out of the set $X$, 
mapping them to a non-negative real number, must satisfy:
\begin{align}
...
\newthought{Euclidean Distance}\index{Euclidean distance} is probably the most familiar distance metric, measuring the straight-line distance between two points in space.
...
\newthought{Euclidean distance} assumes all dimensions are equally important and measured in the same units. It also assumes a continuous, smooth space.
...
\begin{align*}
    \mu &= (4.67,\; 5.00) \\
    \sigma &= (3.08,\; 3.41) \\[4pt]
    z_x &= \frac{3 - 4.67}{3.08} = -0.54 \\
    z_y &= \frac{1 - 5.00}{3.41} = -1.17 \\[4pt]
    A' &= (-0.54,\; -1.17)
\end{align*}}
...
\begin{align*}
    x' &= \frac{3-1}{8-1} = \frac{2}{7} \approx 0.29 \\
    y' &= \frac{1-1}{9-1} = 0 \\
    A' &= (0.29,\; 0)
\end{align*}}
\begin{equation}
...
but is even more sensitive to outliers since a single extreme 
...
\newthought{A coffee quality lab} measures sourness (0--200) and bitterness (0--500) 
for each batch.
...
\newthought{Intuitively,} $A^{-1}$ undoes the transformation $A$: if $A$ stretches and rotates space, $A^{-1}$ maps everything back, with $A A^{-1} = I$. For $\Sigma$, the inverse rescales each dimension by its variance and removes correlations, so distances are measured in standard-deviation units.}
\begin{align*}
    \det(\Sigma) &= 3900 \cdot 15600 - 6600^2 = 17{,}280{,}000 \\[6pt]
    \Sigma^{-1}  &= \frac{1}{17{,}280{,}000}\begin{pmatrix} 15600 & 6600 \\ 6600 & 3900 \end{pmatrix}
\end{align*}

The difference vector is $\delta = P_3 - P_1 = (30,\;{-}180)$. Applying $\Sigma^{-1}$:
\begin{align*}
    \Sigma^{-1}\delta &= \frac{1}{17{,}280{,}000}
...
Taking the dot product with $\delta$ gives the squared distance:
\begin{align*}
    d_M^2 &= \delta^T(\Sigma^{-1}\delta) \\[4pt]
          &= \frac{1}{17{,}280{,}000}(30,\;{-}180)\begin{pmatrix}-720{,}000\\-504{,}000\end{pmatrix} \\[4pt]
          &= \frac{69{,}120{,}000}{17{,}280{,}000} = 4 \\[4pt]
    d_M   &= \sqrt{4} = 2
\end{align*} 
...
\marginnote{$C_1 = (100,\;3)$\\$C_2 = (500,\;7)$\\$C_3 = (300,\;5)$\\[4pt]daily cups sold, customer rating (1--10)}
...
Compute the Euclidean distance on raw features, standardize all three points, and recompute. On raw features, daily sales dominate completely:
\begin{align*}
    d_E(C_1, C_2) &= \sqrt{(500{-}100)^2 + (7{-}3)^2} \\
            &= \sqrt{160000 + 16} \\
            &= \sqrt{160016} \\
            &\approx 400.0
\end{align*}

Daily sales contribute $160{,}000$ out of $160{,}016$ to the squared distance; the rating is drowned out entirely. The means and standard deviations are:
\begin{align*}
    \mu_{\text{cups}} &= 300, & \sigma_{\text{cups}} &= 200 \\
    \mu_{\text{rating}} &= 5,   & \sigma_{\text{rating}} &= 2
\end{align*}
\marginnote{\newthought{This example} was constructed to give clean z-scores. Real data rarely cooperates: means are rarely round, standard deviations are rarely integers, and normalized points land at arbitrary decimals. The principle holds regardless, it's just ugly.}

After z-score normalization the cafés map to:
\begin{align*}
    C_1' &= (-1,\,-1), & C_2' &= (1,\,1), & C_3' &= (0,\,0)
\end{align*}

Recomputing the distance:
\begin{align*}
    d(C_1', C_2') &= \sqrt{(1{-}({-}1))^2 + (1{-}({-}1))^2} \\
              &= \sqrt{4 + 4} \\
              &= \sqrt{8} \\
              &\approx 2.83
\end{align*}

Both features now contribute equally. The distance is driven by how many standard deviations the cafés are apart, not by raw units.
...
\marginnote{20 coffee samples\\acidity (pH, 4.0--7.0)\\bitterness (1--10)\\brew strength (mg/mL, 5--25)\\[4pt]four varieties: cold brew, drip, espresso, latte}

\newthought{A toy dataset to explore.} You are given measurements of 20 coffee samples with three features:
\textit{acidity} (pH, 4.0--7.0), \textit{bitterness} (1--10), and \textit{brew strength} (mg/mL, 5--25).
The samples belong to four varieties: cold brew, drip, espresso, and latte.
Load the dataset and scatter-plot acidity vs.\ bitterness. Then compute the Euclidean distance matrix and identify the most similar pair, is the result plausible? Apply z-score normalization, recompute the distance matrix, and observe how the nearest neighbors shift. Compute cosine similarity between all pairs and compare it to the Euclidean grouping. Finally, compute the sample covariance matrix of the normalized data and check whether any features are correlated.

\newthought{What to observe.} Without normalization, brew strength dominates similarity because its scale (5--25\,mg/mL) dwarfs the others.
