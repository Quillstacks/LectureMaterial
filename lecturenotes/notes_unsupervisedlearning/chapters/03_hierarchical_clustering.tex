...
\newthought{Ich springe von Level zu Level zu Level.}
...
\newthought{The centroid assumption}\index{geometric assumptions} forces every cluster to be convex and compact.\index{cluster shape}
...
\newthought{Sensitivity to initialization and outliers}\index{outliers}\index{initialization!sensitivity} means results are non-deterministic.\index{non-determinism}
...
\begin{marginfigure}
    ...
    \begin{tikzpicture}
        ...
        % All pairwise distances as dashed lines
        \draw[densely dashed, black!25, thin] (axis cs:1,2) -- (axis cs:2,3);
        \draw[densely dashed, black!25, thin] (axis cs:1,2) -- (axis cs:3,1);
        \draw[densely dashed, black!25, thin] (axis cs:1,2) -- (axis cs:6,8);
        \draw[densely dashed, black!25, thin] (axis cs:1,2) -- (axis cs:8,7);
        \draw[densely dashed, black!25, thin] (axis cs:1,2) -- (axis cs:8,9);
        \draw[densely dashed, black!25, thin] (axis cs:2,3) -- (axis cs:3,1);
        \draw[densely dashed, black!25, thin] (axis cs:2,3) -- (axis cs:6,8);
        \draw[densely dashed, black!25, thin] (axis cs:2,3) -- (axis cs:8,7);
        \draw[densely dashed, black!25, thin] (axis cs:2,3) -- (axis cs:8,9);
        \draw[densely dashed, black!25, thin] (axis cs:3,1) -- (axis cs:6,8);
        \draw[densely dashed, black!25, thin] (axis cs:3,1) -- (axis cs:8,7);
        \draw[densely dashed, black!25, thin] (axis cs:3,1) -- (axis cs:8,9);
        \draw[densely dashed, black!25, thin] (axis cs:6,8) -- (axis cs:8,7);
        \draw[densely dashed, black!25, thin] (axis cs:6,8) -- (axis cs:8,9);
        \draw[densely dashed, black!25, thin] (axis cs:8,7) -- (axis cs:8,9);
        % Full single-linkage hierarchy as solid lines (matching trace)
        % Merge 1: x1--x2
        \draw[black!55, thin] (axis cs:1,2) -- (axis cs:2,3);
        % Merge 2: x5--x6
        \draw[black!55, thin] (axis cs:8,7) -- (axis cs:8,9);
        % Merge 3: x4--x5 (x4 joins {x5,x6} via x5)
        \draw[black!55, thin] (axis cs:6,8) -- (axis cs:8,7);
        % Merge 4: x2--x3 (x3 joins {x1,x2} via x2)
        \draw[black!55, thin] (axis cs:2,3) -- (axis cs:3,1);
        % Merge 5: x2--x4 (final merge across groups)
        \draw[black!55, thin] (axis cs:2,3) -- (axis cs:6,8);
        % Data points
        \addplot[only marks, mark=*, mark size=2pt, black!55] coordinates {(1,2)(2,3)(3,1)(6,8)(8,7)(8,9)};
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{The canonical six points with all pairwise distances (dashed) and the full single-linkage hierarchy (solid). 
    The last merge connects $x_2$ and $x_4$ across the two groups. No centroids needed; 
    only pairwise distances drive the clustering.}
...
The full range of partitions\index{partition}, from $K{=}1$ to $K{=}n$, lives in this one structure.
...
\newthought{The agglomerative strategy}\index{agglomerative clustering}\cite{Ward1963} is bottom-up: given data $X = \{x_1,\ldots,x_n\}$, initialize $n$ singleton clusters $C_i = \{x_i\}$ in an active set and precompute a distance matrix\index{distance matrix} $D[i,j] = d(x_i, x_j)$.
...
\newthought{Notice that $L$ is the only hyperparameter to determine.}\index{objective function}
The algorithm is otherwise fully deterministic.\index{determinism}

\newthought{Initialization is deterministic.}\index{initialization!hierarchical clustering} Each point starts as its own singleton cluster,
...
dendrogram.\marginnote{Strictly, the dendrogram is unique only when all pairwise distances are distinct. 
When ties occur, the merge order depends on the tie-breaking rule, typically lowest index first.}
...
non-convex clusters\index{non-convex clusters}. This behavior is called chaining, and is also a weakness; a single close pair of
...
\marginnote{$\|\cdot\|^2$ is a squared norm, not a Euclidean distance. Ward's values are variance increases; single and average linkage values are distances. The scales are not comparable.}
Ward's method produces compact, spherical clusters\index{geometric assumptions} and is closely related to K-Means as
both minimise within-cluster variance\index{within-cluster variance} or inertia. Of course this makes Ward share the
...
\newthought{Irreversible greedy merges and local optima.}\index{local optimum}\index{convergence} At each step the algorithm
...
A large gap between consecutive merge heights is the dendrogram equivalent of the elbow\index{hyperparameter selection}: it suggests a natural number of clusters.
...
under the same distance metric used for clustering.
Or we assign it to a cluster based on the closest leaf or a k-nearest neighbor approach.
...
This matches SLINK\index{SLINK}, the classical optimal algorithm for single linkage, which encodes the same MST structure via a pointer representation without building the full distance matrix.
\fi
...
\begin{marginfigure}
    ...
    \label{fig:hc_exercise_three_scatter}
\end{marginfigure}
...
distance between points or boundaries locally. No criterion has a concept of
sparsity or of low-density regions. In the next Chapter we will introduce and leverage the concept
