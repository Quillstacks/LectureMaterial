...
\begin{tikzpicture}
    ...
    \end{axis}
\end{tikzpicture}
\caption{Na\"ive feature selection: dropping $x^{(2)}$ projects onto the horizontal axis. The dashed errors are large because the data varies along $y{=}x$, not along a coordinate axis. Both features have nearly equal variance, so the na\"ive ranking cannot decide which to keep.}
...
\begin{tikzpicture}
    ...
    \end{axis}
\end{tikzpicture}
\caption{PCA projection onto PC1: the direction of maximum variance runs along $y{=}x$. The reconstruction errors (dashed) are tiny compared to the na\"ive projection.}
...
\begin{tikzpicture}
    ...
    \end{axis}
\end{tikzpicture}
\caption{Scree plot showing individual eigenvalues. The sharp drop after the first component reveals that one direction dominates.}
...
\begin{tikzpicture}
    ...
    \end{axis}
\end{tikzpicture}
\caption{Cumulative explained variance. The dashed line marks the 95\% threshold, reached at six components.}
...
\begin{tikzpicture}
    ...
    \end{axis}
\end{tikzpicture}
\caption{t-SNE embedding at perplexity $= 5$. Three tight clusters emerge from local neighbourhood preservation.}
\label{fig:tsne_perplexity_low}
\end{figure}
...
\begin{tikzpicture}
    ...
    \end{axis}
\end{tikzpicture}
\caption{t-SNE embedding at perplexity $= 50$. The same data produces different cluster shapes, sizes, and between-cluster distances. Neither embedding reflects the original geometry.}
\label{fig:tsne_perplexity_high}
\end{figure}
