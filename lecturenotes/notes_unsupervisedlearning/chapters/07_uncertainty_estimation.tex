...
\chapter{Knowing What You Don't Know}\index{Uncertainty Estimation}\index{Anomaly Detection}\index{OOD Detection}
\newthought{A model that cannot express doubt is dangerous.}
...
\newthought{In the previous chapters} we learned to compress high-dimensional data into lower-dimensional representations and to generate new samples from learned latent spaces.
Neither capability answers a critical question: how much should we trust what the model has learned?
A VAE that reconstructs a familiar coffee aroma profile faithfully tells us nothing about what it would do with a contaminated sample it has never seen.

\newthought{Uncertainty estimation} closes this gap.
...
The answer matters wherever consequences are real: medical diagnosis, autonomous driving, fraud detection, manufacturing quality control.
...
\newthought{Given} a cluster of training points, consider the three grey candidates.
How confident should a model be about each one?
What sources of uncertainty are at play, noise in the data or gaps in the model's experience?
How would you decide when the model's judgment can be trusted and when a human should intervene?
...
\newthought{Monte Carlo Dropout}\cite{GalGhahramani2016} estimates epistemic uncertainty by running $T$ forward passes with dropout enabled at test time.
...
Unlike MC Dropout, each ensemble member is initialised and trained independently, so the diversity of predictions is driven by genuine differences in the learned representations rather than by random masking.


\section{Application: Anomaly Detection}\index{Anomaly Detection}\index{outlier detection}

\marginnote{\newthought{One of the most direct applications} of uncertainty estimation is anomaly detection: when a model assigns high uncertainty to an input, it is effectively saying that this does not look like what it was trained on.}
...
\subsection{Types of Anomalies}\index{point anomaly}\index{contextual anomaly}\index{collective anomaly}
...
\subsection{Statistical Methods}\index{Z-score}\index{GMM!anomaly detection}
...
\subsection{Proximity-Based Methods}\index{k-NN distance}\index{Local Outlier Factor}
...
\subsection{Reconstruction-Based Methods}\index{reconstruction error}\index{VAE!anomaly detection}
...
\begin{align*}
    ...
    \text{Var} &= \tfrac{1}{5}\sum_t (p_t - 0.66)^2 \\
               &\approx 0.034
\end{align*}%
The mean leans toward cat, but the variance is substantial; pass 2 disagrees strongly. This input sits in a region of high epistemic uncertainty, and routing it to a human reviewer would be prudent.
}
...
\begin{itemize}
    \item Uncertainty comes in two forms: aleatoric, which is irreducible noise in the data, and epistemic, which reflects the model's ignorance and shrinks with more data.
    \item MC Dropout and deep ensembles quantify epistemic uncertainty by measuring disagreement across multiple predictions; high variance flags inputs the model has not learned to handle.
    \item Anomaly detection applies uncertainty reasoning through statistical, proximity-based, and reconstruction-based scores; the type of anomaly, whether point, contextual, or collective, determines which method is appropriate.
    \item OOD detection formalises the question of whether an input belongs to the training distribution at all; energy-based scores outperform naive softmax confidence.
    \item Graceful degradation, combining confidence thresholds, human handoff, fail-safe defaults, and distribution-shift monitoring, turns uncertainty estimates into a practical safety net.
\end{itemize}

\marginnote{\newthought{The fundamental limit} of every method in this chapter: each relies on a model of normal trained without labels. When the boundary of normal is ambiguous, no uncertainty estimate can substitute for domain knowledge.}

\newthought{Every uncertainty method in this chapter} inherits the model of normality from the training data.
...
\marginnote{\newthought{Teaser.} Can we learn representations directly from the structure of the data, using the data itself as supervision, so that clustering, anomaly detection, and downstream tasks all benefit?}
