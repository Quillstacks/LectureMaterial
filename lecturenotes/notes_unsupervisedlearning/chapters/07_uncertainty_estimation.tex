...
\marginnote{\newthought{Neural networks} are trained to predict, and they do so with impressive accuracy on familiar data, which makes them particularly misleading on unfamiliar data.}

\newthought{In the previous chapter} we learned to compress high-dimensional data into structured latent spaces and to draw new samples from those spaces.
The Variational Autoencoder mapped each input to an embedding, decoded it back, and could even synthesise plausible new samples from the prior.
Neither capability told us how much we should trust those outputs for any particular input.
A reconstruction looks the same whether the input lay in the heart of the training distribution or far from any encoded neighbour in feature space; the model has no built-in way to communicate its own uncertainty.

\marginnote{\newthought{The long tail.}\cite{Koopman2018HeavyTail} Real-world data is heavy-tailed: a small number of common situations cover most of the volume, but a long tail of rare configurations refuses to vanish no matter how much data is collected. Autonomous vehicles see most of the road most of the time and meet the unusual every few hours, and a model that is confident on the head will be confident on the tail by default.}

\newthought{Models trained on finite data are confident outside their training distribution by default.}
The softmax of a classifier sums to one regardless of whether the input is a familiar digit or a sample drawn from pure noise.
The decoder of a VAE produces a smooth output regardless of whether the latent code corresponds to a region the encoder ever visited.
Confidence is a number the model returns; it is not a flag that the model has actually seen anything like this input before, nor a measure of how far the input sits from the embeddings of the training data.

\newthought{Silent failure is the natural consequence.}
A model that cannot express doubt will eventually be confidently wrong, and no downstream safeguard can compensate for an uncertainty signal that was never produced in the first place.
The classifier returns its prediction, the planner trusts it, the actuator executes; the loop closes around the wrong answer with no visible alarm.

\newthought{Acting under uncertainty} is what turns a raw doubt signal into a deployable safety net.
Three operational layers compose to keep the loop open.
Human-in-the-loop puts a reviewer on the critical path: the model emits a prediction with an uncertainty score, and the system stalls until the reviewer accepts or overrides.
Human-on-the-loop runs the model autonomously and escalates only flagged cases, so the reviewer steers without blocking the average prediction.
Fail-safe defaults are the conservative action taken when the model has flagged the input as outside its competence and no reviewer is available.
Behind all three is monitoring, the slow loop that watches the distribution of scores over time and alerts when the input world has drifted from the world the model was trained on.
...
\newthought{Confidence is not correctness.}
Neural networks calibrate poorly on out-of-distribution inputs by design: the softmax normalises to sum to one whether the input is a training-set image or a frame from a film the network has never seen.
...
The cure is not to make the model more confident; it is to make the model honest about what it does not know, so that confidence and accuracy track each other on a calibration plot rather than drifting apart.
A complementary route is to train a small head whose only job is to predict whether the main model will be right on the current input, turning the model's own likelihood of being correct into a learned routing signal that sends the cases it is likely to mishandle to a reviewer before they cause damage.

\marginnote{\newthought{Calibration plot.}\cite{Guo2017Calibration} Bin predictions by stated confidence, then plot mean confidence on the horizontal axis against empirical accuracy on the vertical. The diagonal is perfect calibration; modern deep classifiers typically sit above the diagonal, which is to say they are systematically overconfident.}

\newthought{Different sources of uncertainty call for different responses.}
Some uncertainty is irreducible noise in the data and will not shrink with any amount of training.
Some uncertainty reflects gaps in what the model has been shown and shrinks the moment relevant data arrives.
Treating the two the same wastes effort and conceals the actionable signal under the inevitable one.

\marginnote{\newthought{ODD and drift.}\index{operational design domain}\index{distribution shift} The operational design domain names the conditions under which a system was designed to work, namely the slice of the world the training data covered. ODD detection asks at runtime whether the current input falls inside that slice. Data drift is the slow change in input statistics over time; model drift is the resulting decoupling of stated confidence from empirical accuracy. Both bend the calibration plot away from the diagonal long before the operator notices.}

\newthought{Uncertainty estimation closes the gap.}
It asks: given a model trained on finite data under specific assumptions, how reliable are its outputs for a particular input, and what kind of unreliability is at play?
The answer turns a single output into a distribution over outputs, a single confidence number into a structured signal that downstream code can route on.
A close cousin of the same problem is anomaly detection trained as one-class learning\index{one-class learning}: deliberately fit only normal data, then flag any input the model finds unusual without ever showing it a labelled anomaly.
The same uncertainty signal that powers calibration also powers detection of inputs the training distribution never covered, which is the regime where labels are rare and the events that matter most are the ones nobody has annotated.
...
\newthought{Aleatoric uncertainty}\index{aleatoric uncertainty} is inherent noise in the data and cannot be reduced by collecting more of it.
...
\newthought{Monte Carlo Dropout}\index{Monte Carlo Dropout}\cite{GalGhahramani2016} estimates epistemic uncertainty by running $T$ forward passes through the trained network with dropout enabled at test time.
...
Gal and Ghahramani showed that this matches a variational approximation to a Bayesian neural network with a specific prior over weights, so MC Dropout can be read as a cheap, after-the-fact approximation of a posterior we never explicitly trained.
...
\marginnote{\newthought{The hard part is calibration.} Even a well-quantified variance can be miscalibrated if the model overestimates its certainty. Temperature scaling, Platt scaling, and isotonic regression are post-hoc corrections that align stated probabilities with empirical frequencies. None of them shrink epistemic uncertainty; they make the estimate honest.}
...
\newthought{VAE log-likelihood} refines the idea by using the negative ELBO from chapter 6 as the anomaly score:
...
\newthought{Softmax confidence} is the naive baseline: flag inputs where $\max_k\, p(y{=}k \mid x)$ is low.
...
\newthought{A separate detector head} pushes the same idea one rung further by training an auxiliary binary head on top of the existing backbone\cite{Chandola2009}, with the explicit job of predicting in-distribution membership rather than class identity.
...
\newthought{Distribution shift} is the most common and the hardest to detect.
...
\newthought{Calibration drift} compounds the shift problem.
...
\newthought{Reconstruction-based blindness} hits a different surface.
...
\newthought{The threshold problem} is the practical headache that haunts every method.
...
\begin{tikzpicture}
    ...
    \end{axis}
\end{tikzpicture}
\caption{Receiver operating characteristic for an anomaly detector. The curve sweeps the threshold from strict (low FPR, low TPR) to loose (high FPR, high TPR). The open marker is one operating point chosen by domain cost: a false positive costs a human review; a false negative costs an undetected anomaly.}
\label{fig:roc_curve}
\end{figure}

\marginnote{\newthought{Stability analysis.} Re-train the same detector on bootstrap subsamples of the training set and compare the flagged sets. Stable flags suggest genuine anomalies; flags that appear in only a few re-runs reflect detector variance, not signal. Stability is also a useful drift detector: a sudden drop in flag stability over time often precedes calibration failure.}
...
\newthought{Confidence thresholds} are the first layer.
...
\newthought{Human handoff} is the second layer.
...
\newthought{Fail-safe defaults} are the third layer.
...
\newthought{Monitoring} is the fourth layer, and the one most often skipped.
...
\begin{itemize}
    ...
    \item When would you prefer deep ensembles over MC Dropout, and what are the trade-offs in compute and quality?
    ...
    \item When does $k$-NN distance fail in a way that LOF handles, and what is the underlying mechanism?
    \item Why might an autoencoder trained on normal data reconstruct an anomaly with low error, and how would you mitigate this?
    \item Why is high softmax confidence not a reliable indicator that an input is in-distribution, and what does the energy score see that softmax does not?
    \item How does distribution shift cause an uncertainty detector to silently degrade, and what are the operational signals that the detector is no longer trustworthy?
    \item Why should a deployed model include both a fail-safe default and a human-handoff mechanism, and how do you decide which branch is appropriate for a given uncertainty level?
\end{itemize}

\newthought{Recap} of Key Concepts:
\begin{itemize}
    \item Uncertainty comes in two forms: aleatoric, which is irreducible noise in the data, and epistemic, which reflects the model's ignorance and shrinks as more data arrives. Only the latter is actionable through more training data.
    \item MC Dropout and deep ensembles both quantify epistemic uncertainty by measuring disagreement across multiple predictions. MC Dropout is cheap to deploy on a single trained network; deep ensembles cost $M$ training runs but produce more reliable uncertainty estimates on truly novel inputs.
    \item Scoring methods turn uncertainty into decisions through statistical, proximity, and reconstruction families, plus OOD-specific scores. The type of anomaly, point, contextual, or collective, dictates which family is appropriate, and the threshold is a cost decision rather than a statistical one.
    \item Energy-based OOD detection separates in- from out-of-distribution inputs more reliably than softmax confidence because energy is not normalised; the absolute scale of the logits carries information that the softmax discards.
    \item Every uncertainty method inherits its model of normal from the training data, and that data drifts. Calibration, distribution shift, and reconstruction blindness are predictable failure modes that operational monitoring is designed to catch before they cause silent failures.
    \item Graceful degradation composes confidence thresholds, fail-safe defaults, human handoff, and monitoring into a safety net. A deployed model that can refuse to answer the cases it should refuse on is more trustworthy than one that always answers.
\end{itemize}

\marginnote{\newthought{Root cause.} Every method in this chapter inherits its notion of normal from a narrow training distribution, and the long tail of novel inputs is structurally invisible to a narrowly-trained detector.}

\newthought{Every uncertainty method in this chapter} inherits its model of normality from the training data, and that data covers only a sliver of the input space the deployed model will encounter.
A fraud detector trained on last year's transaction patterns will keep waving through this year's novel scheme until enough damage accumulates to relabel the data and retrain.
The detector still works as designed; it just no longer works for the world it now lives in, and there is nothing inside the detector itself that flags this, because the failure lives outside the feature space it was ever shown.

\newthought{Transfer learning} addresses this brittleness by widening the feature space before the task ever arrives.
Instead of training a narrow detector on a narrow dataset, we reuse a backbone that has already been pre-trained on web-scale data, so that the long tail of any one task already sits inside the head of the backbone's representation~\cite{Radford2021CLIP}.
Foundation models such as CLIP, GPT, and SAM push this idea to the extreme: a single representation, learned once on hundreds of millions of image-text pairs or tokens, can be reused on classification, retrieval, or segmentation tasks it was never explicitly trained for, with zero or only a handful of labelled examples.

\marginnote{\newthought{Teaser.} How can a sentence like ``a photo of a cat'' act as a classifier for an image the model has never seen before?}

\newthought{The next chapter explores} how pre-trained vision-language models turn a text prompt into a zero-shot classifier, how few-shot fine-tuning, linear probing, and adapters reuse a frozen backbone on a new task, and what goes wrong when the source and target domains drift far enough apart that transfer becomes negative.
