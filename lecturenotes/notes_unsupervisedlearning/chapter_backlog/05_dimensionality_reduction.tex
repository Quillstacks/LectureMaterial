...
Yet the \textbf{intrinsic dimensionality}, the number of degrees of freedom that actually vary, is often much lower.
...
\newthought{Why reduce dimensions?}
\begin{itemize}
    \item \textbf{Visualization}: we can only see 2D or 3D directly.
    \item \textbf{Noise reduction}: irrelevant variation is filtered away.
    \item \textbf{Computational efficiency}: downstream algorithms run faster and generalize better.
    \item \textbf{Feature extraction}: latent factors of variation are made explicit.
    \item \textbf{Combating the curse of dimensionality}: distances become meaningful again.
\end{itemize}
...
\marginnote{\newthought{Richard Bellman} coined the term ``curse of dimensionality'' in 1961 while studying dynamic programming. The insight carries over directly to machine learning.}
...
\begin{tikzpicture}
    ...
    \addplot[black, thin, domain=1:20, samples=20]
...
\marginnote{\newthought{The blessing of dimensionality.} The Johnson-Lindenstrauss lemma shows that $n$ points in high dimensions can be projected to $O(\log n / \epsilon^2)$ dimensions while preserving pairwise distances within $(1{\pm}\epsilon)$. Random projections are cheap and surprisingly powerful.}
...
\marginnote{\newthought{PCA} finds orthogonal directions of maximum variance in the data.}

\newthought{Given centred data} $X \in \mathbb{R}^{n \times d}$ (rows are samples), PCA finds a $k$-dimensional subspace ($k < d$) that best represents the data.
...
where $U \in \mathbb{R}^{n \times r}$ holds left singular vectors (orthonormal), $\Sigma \in \mathbb{R}^{r \times r}$ is diagonal with singular values $\sigma_1 \geq \sigma_2 \geq \ldots$, $V \in \mathbb{R}^{d \times r}$ holds right singular vectors (orthonormal), and $r{=}\text{rank}(X)$.
...
By the Eckart-Young-Mirsky theorem, $X_k$ is the best rank-$k$ approximation to $X$ in both Frobenius and spectral norms.
...
ICA goes further and finds \textbf{independent} components.
...
\newthought{The key insight:} independent non-Gaussian signals become more Gaussian when mixed (central limit theorem).
ICA maximises non-Gaussianity, measured via kurtosis or negentropy, to separate sources.
The FastICA algorithm implements this efficiently.


\section{t-SNE}\index{t-SNE}

\marginnote{\newthought{t-SNE} stands for t-distributed Stochastic Neighbour Embedding (van der Maaten \& Hinton, 2008). It is optimised for two- or three-dimensional visualization.}
...
\begin{itemize}
    \item Cluster \textbf{sizes} are meaningless: t-SNE equalises local densities.
    \item \textbf{Distances between clusters} are meaningless: the embedding preserves local, not global, structure.
...
\marginnote{\newthought{UMAP} stands for Uniform Manifold Approximation and Projection (McInnes et al., 2018). It has theoretical foundations in Riemannian geometry and algebraic topology.}
...
\begin{enumerate}
    ...
    \item Use cross-entropy loss (not KL divergence) to match fuzzy simplicial sets.
\end{enumerate}

\newthought{Key parameters.} \texttt{n\_neighbors} (similar to perplexity, typically 15--200) controls how much local versus global structure is preserved.
